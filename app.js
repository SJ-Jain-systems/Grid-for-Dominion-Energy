...
function getPriorityKey(){const s=document.getElementById('strategyFilter').value;return s==='grid'?'gridPriority':s==='affordability'?'affordabilityPriority':'adoptionPriority';}
...
function renderMap(countyStats){
  const svg=document.getElementById('vaMap');
  const vals=countyStats.map(c=>c.vulnerability); const min=Math.min(...vals),max=Math.max(...vals);
  const color=v=>{const t=(v-min)/(max-min||1);return t>0.66?'#cb2b3e':t>0.33?'#f39c3d':'#5e9df6';};
  svg.innerHTML=`<path d="M40,220 L120,150 L240,130 L360,80 L520,140 L700,170 L860,210 L810,250 L620,250 L470,235 L320,250 L160,270 L80,250 Z" fill="#eef4ff" stroke="#b4c6ea"/>`+
  countyStats.map(c=>{const p=countyPos[c.county]||[60,60];return `<g><circle cx='${p[0]}' cy='${p[1]}' r='14' fill='${color(c.vulnerability)}' stroke='#1b3f87'/><text x='${p[0]+18}' y='${p[1]+4}' font-size='12'>${c.county}</text></g>`;}).join('');
}

function render(){
  const key=getPriorityKey();
  const rows=filters(households.map(recommend).sort((a,b)=>b[key]-a[key]));
  const topRows=rows.slice(0,TABLE_LIMIT);
  const high=topRows.filter(r=>r[key]>topRows[0]?.[key]*0.7).length;
  const kpis=[['Households analyzed',rows.length],['High-priority households',high],['Average energy burden',`${(topRows.reduce((s,r)=>s+r.energyBurden,0)/Math.max(topRows.length,1)).toFixed(1)}%`],['Projected peak reduction',`${(topRows.reduce((s,r)=>s+r.peak57*0.09,0)).toFixed(1)} MW`],['Estimated annual savings',`$${Math.round(topRows.reduce((s,r)=>s+r.affordability*12*0.05,0)).toLocaleString()}`],['Top segment',topRows[0]?`${topRows[0].energyBurden>7?'High energy burden':'Moderate burden'} households`: 'n/a']];
  ...
  const countyStats=groupBy(rows,'county').map(([county,list])=>({county,count:list.length,avgPriority:list.reduce((s,r)=>s+r[key],0)/list.length,vulnerability:list.reduce((s,r)=>s+r.energyBurden+r.outageRisk*10+r.peak57*0.5,0)/list.length,topTech:mode(list.map(r=>r.tech))})).sort((a,b)=>b.vulnerability-a.vulnerability);
...
async function init(){households=await (await fetch('data/virginia_sample.json')).json();[...new Set(households.map(h=>h.county))].sort().forEach(c=>{const o=document.createElement('option');o.value=c;o.textContent=c;countyFilter.appendChild(o);});render();}
const countyFilter=document.getElementById('countyFilter'),incomeFilter=document.getElementById('incomeFilter'),derFilter=document.getElementById('derFilter'),goalFilter=document.getElementById('goalFilter'),minAdoption=document.getElementById('minAdoption'),rowsEl=document.getElementById('rows'),detail=document.getElementById('detail'),geo=document.getElementById('geo'),kpiCards=document.getElementById('kpiCards');
